...
| APP: chain-indexer | Postgres | `-rw` and `-ro` hostnames, port 5432, database `indexer-db` (`APP_CHAIN_INDEXER_DB_NAME`), with a connection URI and a `psql` command for each |
