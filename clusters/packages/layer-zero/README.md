...
| `OP_VAULT_GATEWAY_LB_SOURCE_RANGES` | client CIDRs allowed to open a connection to the gateway's load balancer at all, stored as a YAML flow sequence, e.g. `'["0.0.0.0/0"]'`; enforced by the cloud firewall on the real source address, including direct connections to node ports; this is the only IP allowlist, covering every route on the gateway (Grafana, GraphQL, Postgres) |
