...
`s3_backup.tf` creates `oatlabs-backoffice-pg-backups-staging` and attaches a policy scoped to it onto the
...
`s3_replica.tf` replicates the bucket to `oatlabs-backoffice-pg-backups-staging-replica`
